...
server.DB_PATH. They cover the window math layered on top of db.search —
...
def test_explicit_date_range_bounds_inclusive(seeded_db):
    titles = {e["title"] for e in server.search_events(
...
def test_start_date_with_days_ahead_width(seeded_db):
    ...
    assert server.search_events(start_date="2026-08-13", days_ahead=1) == []


def test_end_before_start_raises(seeded_db):
    with pytest.raises(ValueError):
        server.search_events(start_date="2026-08-15", end_date="2026-08-13")


def test_bad_date_format_raises(seeded_db):
    with pytest.raises(ValueError):
        server.search_events(start_date="08/13/2026")
